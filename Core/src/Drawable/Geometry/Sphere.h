...
namespace D3DEngine
{
	...
	{
		public:
			static IndexedTriangleList MakeTesselated(DynamicVertexLayout layout, int latDiv, int longDiv)
			{
				...
				RawVertexBufferWithLayout vb{std::move(layout)};
				...
				const auto iSouthPole = (unsigned short)vb.Size();
				{
					...
					XMStoreFloat3(&southPos, dx::XMVectorNegate(base));
			...
			static IndexedTriangleList Make(std::optional<DynamicVertexLayout> layout = std::nullopt)
			{
				using Element = DynamicVertexLayout::ElementType;
				if (!layout)
				{
					layout = DynamicVertexLayout{}.Append(Element::Position3D);
				}
