	...
	phi(adist(rng))
{
	...
	{
		auto pvs = std::make_unique<VertexShader>(gfx, L"Shaders/cso/ColorIndexVS.cso");
		auto pvsbc = pvs->GetBytecode();
		AddStaticBind(std::move(pvs));

		AddStaticBind(std::make_unique<PixelShader>(gfx, L"Shaders/cso/ColorIndexPS.cso"));
	...
	// Every instance has a different mesh
	struct Vertex
	{
		dx::XMFLOAT3 pos;
	};
