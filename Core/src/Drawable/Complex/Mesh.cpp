...
Mesh::Mesh(Graphics& gfx, std::vector<std::unique_ptr<Bindable>> bindPtrs)
{
	...
	{
		// assume that all models are triangle list
	...
	for (auto& pb : bindPtrs)
	{
		if (auto pi = dynamic_cast<IndexBuffer*>(pb.get()))
		{
			...
			// tell original index buffer to release the memory (since it's now being transferred to other places)
			pb.release();
		}
		else
		{
